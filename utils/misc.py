...
class gdlLoss(nn.Module):
    ...
    def forward(self,inputs,targets):
        t1 = torch.abs(inputs[:,:,1:,:] - inputs[:,:,:-1,:]) - torch.abs(targets[:,:,1:,:] - targets[:,:,:-1,:])

        t2 = torch.abs(inputs[:,:,:,:-1] - inputs[:,:,:,1:]) - torch.abs(targets[:,:,:,:-1] - targets[:,:,:,1:])
        t1 = torch.abs(t1)
        t2 = torch.abs(t2) 
        result = torch.sum(torch.mean(t1,0)) + torch.sum(torch.mean(t2,0))
        return result
